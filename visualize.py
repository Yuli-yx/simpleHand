...
def get_abs_joints3d(joints3d, gamma, K, ROOT_INDEX=9, image_shape=224):
    camera_relative_k = np.sqrt(K[0][0] * K[1][1] * (0.2**2) / (image_shape**2))
    root_depth = gamma * camera_relative_k
    
    root_coord = joints3d[ROOT_INDEX].copy()
    root_coord[2] = root_depth
    
    joints3d = joints3d + root_coord[None, :]
    joints3d[ROOT_INDEX] = root_coord
...
def infer_single_json(val_cfg, bmk, model, rot_angle=0):
    ...
    for cur_iter, batch_data in enumerate(tqdm(dataloader)):
        ...
        joints_pred = joints[0].cpu().numpy()
        joints_gt = batch_data['xyz'][0].cpu().numpy()
        
        joints_3d_abs = get_abs_joints3d(joints_pred, root_depth[0].cpu().numpy(), K[0].cpu().numpy())
        
        vis_img, pred_2d = project_joints(joints_3d_abs, K[0].cpu().numpy(), img.copy())
        cv2.imwrite("pred_0.png", vis_img)
        print(joints_3d_abs)
        print(joints_gt-joints_gt[ROOT_INDEX][None, :])
        print('diff:', joints_3d_abs - joints_gt)
        # vis_img, pred_2d = project_joints(joints_pred, K[0].cpu().numpy(), img.copy())
        # cv2.imwrite("pred_0.png", vis_img)
        # vis_img2, gt_2d = project_joints(batch_data['xyz'][0].cpu().numpy(), K[0].cpu().numpy(), img.copy(), color=(255, 0, 0))
        # cv2.imwrite("gt_0.png", vis_img2)
        
        # print("2d diff", pred_2d)
        return 
    ...
    return pred_uv_list, pred_joints_list, gt_joints_list


def project_joints(joints_3d, K, img, color=(0, 255, 0), thickness=2):

    project_joints = (K @ joints_3d.T).T # nx3
    project_joints = project_joints[:, :2] / project_joints[:, 2:]
    project_joints = project_joints.astype(np.int32)
    for i in range(joints_3d.shape[0]):
        cv2.circle(img, (project_joints[i, 0], project_joints[i, 1]), 2, color, thickness)
    return img, project_joints
...
def main(epoch, tta=False, postfix=""):
    ...
    for idx in range(len(xyz_gt_list)):
        ...
        xyz = np.array(xyz_gt_list[idx])
        xyz_pred = np.array(xyz_pred_list[idx])
